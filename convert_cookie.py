...
Chuyển đổi cookie Facebook sang nhiều định dạng
"""
...
class CookieConverter:
    ...
    def to_headerstring_format(self, cookies):
        """Convert cookies dict to header string format"""
        ...
        for data in self.cookies_data:
            try:
                ...
                with open(os.path.join(user_dir, "Cookie_Headerstring.txt"), "w") as f:
                    f.write(self.to_headerstring_format(cookies))
                with open(os.path.join(user_dir, "Cookie_NetScape.txt"), "w") as f:
                    f.write(self.to_netscape_format(cookies))
                with open(os.path.join(user_dir, "Cookie_Json.txt"), "w") as f:
...
class CookieConverterApp:
    ...
    def setup_ui(self):
        ...
        # Header
        tk.Label(main, text="🍪 ConvertCookie", font=("Segoe UI", 13, "bold"), 
                bg=Style.BG, fg=Style.TEXT).pack()
        tk.Label(main, text="Chuyển đổi cookie sang Netscape & JSON", 
        ...
        tk.Button(file_row, text="Chọn", command=self.browse_file, font=("Segoe UI", 8),
        ...
        tk.Button(btn_frame, text="⚡ Export Nhanh", command=self.quick_export,
        ...
        tk.Button(btn_frame, text="🔄 Chuyển đổi", command=self.convert_cookies,
                 bg=Style.SUCCESS, fg="white", font=("Segoe UI", 9),
                 relief="flat", padx=12, pady=4, cursor="hand2").pack(side=tk.LEFT, padx=2, expand=True, fill=tk.X)
        
        tk.Button(btn_frame, text="📂 Mở Output", command=self.open_output,
    ...
    def browse_file(self):
        path = filedialog.askopenfilename(title="Chọn file cookie", initialdir=self.input_dir,
    ...
    def quick_export(self):
        ...
        if os.path.exists(default):
            ...
            messagebox.showerror("Lỗi", f"Không tìm thấy: {default}")
            
    def convert_cookies(self):
        path = self.input_var.get()
        if not path or not os.path.exists(path):
            messagebox.showerror("Lỗi", "Chọn file input!")
            return
        ...
        try:
            ...
            self.log(f"📊 Tìm thấy {count} cookie")
            
            if count == 0:
                self.log("⚠️ Không có cookie hợp lệ!")
                self.status_var.set("Không có cookie")
                return
            ...
            self.log("=" * 40)
            self.log(f"✅ THÀNH CÔNG: {results['success']} cookies")
            self.log(f"📁 Output: {os.path.basename(results['output_dir'])}")
            self.log(f"📄 cookies_netscape.txt")
            self.log(f"📄 cookies.json")
            self.log("=" * 40)
            
            self.status_var.set(f"✅ Xong! {results['success']} cookies")
